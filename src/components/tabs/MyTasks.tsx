...
export const MyTasks: React.FC = () => {
  ...
  const handleReassign = async (task: any, newAssigneeId: string) => {
    if (!newAssigneeId || newAssigneeId === task.assignedTo) return;
    await transferTask(newAssigneeId, 'Inline Reassigned from Task List', task);
  };
  ...
  const handleFollowUp = async (task: any, newDateStr: string) => {
    ...
    try {
      ...
      const auditLog = {
        ...
        action: 'Task Follow-Up Date Modified',
        details: `Follow-up date changed from ${task.dateKey || 'Not set'} to ${newDateStr}`
      };
      const auditTrail = [...(task.auditTrail || []), auditLog];
    ...
    <div className="space-y-6">
      ...
      <div className="hidden print:block space-y-6 text-left">
        <div className="print-header flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">DHLC Operations Work Order Report</h1>
            <p className="text-xs text-slate-600 mt-1">Generated by: {user?.displayName || user?.email || 'Operations Admin'} | Active Filter: {activeStatsFilter} Tasks</p>
          </div>
          <div className="text-right text-xs text-slate-600">
            <p className="font-mono font-bold">Date: {todayDateKey}</p>
            <p className="font-mono mt-0.5">Total Records: {displayedTasks.length}</p>
          </div>
        </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '25%' }}>Instructions / Scope</th>
                </tr>
              </thead>
              <tbody>
                {group.tasks.map(t => {
                  ...
                  return (
                    <tr key={t.taskId}>
                      ...
                      <td>{t.assignedToName || 'Department Group'}</td>
                      <td>
